...
<!-- begin breadcrumb -->
    ...
    <!-- end breadcrumb -->
    <!-- begin page-header -->
    <h1 class="page-header">Inbound call<small> You may add Inbound call detail here...</small></h1>
    <!-- end page-header -->
    <!-- begin row -->
	...
	<h4 class="panel-title">Add Inbound Call</h4>
	</div>
	<div class="panel-body">
	    <form <?php if($mode=="add"){?> action="<?php echo site_url('IcomCtr/Service_Edit/add');?>" <?php } else { ?> action="<?php echo site_url('IcomCtr/Service_Edit/edit/'.$ServiceEdit[0]['SH_SYS_ID']);?>" <?php } ?> id="form_validation"  enctype="multipart/form-data" method="POST">
		<!--well start-->
		<div class="well">
			...
			<div class="col-md-6 ui-sortable">
				...
				<!--<div class="row">
					...
					<label class="no-margin radio-inline">
				     ...
				     <section class="col-sm-6 form-group New hide customer">       
				...
				<div class="row">
					...
					<input type="hidden" name="SH_CUST_ID" class="form-control input-sm" value="<?php if($mode=='edit') {echo $ServiceEdit[0]['SH_CUST_ID'];}?>" id="SH_CUST_ID" placeholder="" />
			...
			<div class="col-md-6 ui-sortable">
				...
				<div class="row">
					...
					<input type="text" name="SH_ESTIMATED_AMT" class="form-control input-sm" value="<?php if($mode=="add"){}else{echo $ServiceEdit[0]['SH_ESTIMATED_AMT'];}?>" id="SH_ESTIMATED_AMT"/>
				    </section>
				    <section class="col-sm-6 form-group">       
					<label class="control-label">Amount Received</label>
					<input type="text" name="SH_AMT_RECVD" class="form-control input-sm" value="<?php if($mode=="add"){}else{echo $ServiceEdit[0]['SH_AMT_RECVD'];}?>"  id="SH_AMT_RECVD" />
				    </section>
				</div>
				<div class="row">
				    <section class="col-sm-6 form-group">
				    ...
				    <section class="col-sm-6 form-group">
					<label class="control-label">Next Commit Date</label>					     
					<span class='input-group'>
				...
				<div class="row">
					...
					<div class="form-group col-md-3 switch">
					    <input type="checkbox" class="checkbox" <?php if($ServiceEdit[0]['SH_CUST_APPR_YN']=="Y") echo 'checked="checked"';?>>
					    <input type="hidden" id="SH_CUST_APPR_YN" name="SH_CUST_APPR_YN" value="N">
					</div><?php } ?>
				    </section>
				    <section class="col-sm-4 form-group">
					<label class="control-label">Inform to Customer</label>
					<?php if($ServiceEdit[0]['SH_INFORM_CUST_YN']=="Y") { ?>
					<div class="form-group col-md-3 switch1 switchOn"> 
					    <input type="checkbox" class="checkbox" <?php if($ServiceEdit[0]['SH_INFORM_CUST_YN']=="Y") echo 'checked="checked"';?> >
					    <input type="hidden" id="SH_INFORM_CUST_YN" name="SH_INFORM_CUST_YN" value="<?php echo $ServiceEdit[0]['SH_INFORM_CUST_YN']?>">
					</div><?php } else { ?>
					<div class="form-group col-md-3 switch1">
					    <input type="checkbox" class="checkbox" <?php if($ServiceEdit[0]['SH_INFORM_CUST_YN']=="Y") echo 'checked="checked"';?>>
					    <input type="hidden" id="SH_INFORM_CUST_YN" name="SH_INFORM_CUST_YN" value="N">
					</div><?php } ?>
				    </section>
				    <section class="col-sm-4 form-group">
					<label class="control-label newClass">Delivery Status</label>
					<?php if($ServiceEdit[0]['SH_DELIVERY_YN']=="Y") { ?>
					<div class="form-group col-md-3 switch2 switchOn"> 
					    <input type="checkbox" class="checkbox" <?php if($ServiceEdit[0]['SH_DELIVERY_YN']=="Y") echo 'checked="checked"';?> >
					    <input type="hidden" id="SH_DELIVERY_YN" name="SH_DELIVERY_YN" value="<?php echo $ServiceEdit[0]['SH_DELIVERY_YN']?>">
					</div><?php } else { ?>
					<div class="form-group col-md-3 switch2">
					    <input type="checkbox" class="checkbox" <?php if($ServiceEdit[0]['SH_DELIVERY_YN']=="Y") echo 'checked="checked"';?>>
					    <input type="hidden" id="SH_DELIVERY_YN" name="SH_DELIVERY_YN" value="N">
					</div><?php } ?>
				    </section>
				</div>
				<div class="row">
				    <section class="col-sm-6 form-group">       
					<input type="hidden" name="SH_CALL_TYPE" class="form-control input-sm" value="Inbound" id="SH_CALL_TYPE" />
			...
			<table class="table table-bordered">
				...
				<tr class="odd hidden" id="hiddenRow">
				    <td><input type="hidden" name="LINE_TYPE[]" value="new"><input disabled type="text" name="SL_LINE_NO[]" class="form-control input-sm"></td>
					...
					<select disabled name="SL_PROD_TYPE[]" id="SL_PROD_TYPE" class="form-control table_input selectpicker" data-live-search="true" data-style="btn-white btn-sm" data-container="body">
					    <option selected="" disabled="">Select</option>
					    <?php
					    foreach ($Product as $row)
					    { ?>
				    ...
				    <td><input disabled type="text" name="SL_ISSUE_FROM_CUST[]" class="form-control table_input"></td>
				    <td><input disabled type="text" name="SL_FOUND_BY_EMP[]" class="form-control table_input"></td>
				    <td><input disabled type="text" name="SL_FEEDBACK[]" class="form-control table_input"></td>
				    <td><center><button type="button" onclick="removerow($(this));" class="btn btn-remove btn-danger btn-sm"><i class="fa fa-trash fa-1x"></i></button></center></td>
				</tr>
			    </tbody>
			</table>
...
//    });
	...
	$(this).toggleClass("switchOn");
	if($(this).hasClass('switchOn')){
	   $('#SH_INFORM_CUST_YN').val('Y');
	}
	else{
	   $('#SH_INFORM_CUST_YN').val('N');
	}
    });
	...
	$(this).toggleClass("switchOn");
	if($(this).hasClass('switchOn')){
	   $('#SH_DELIVERY_YN').val('Y');
	}
	else{
	   $('#SH_DELIVERY_YN').val('N');
	}
    });
