...
function update_top_cart(){
		   $.ajax({
			   type: 'POST',
			   url: $('body').data('abs_client_path')+'BurtlanCtr/index',
...
function update_cart(){
		   $.ajax({
			   type: 'POST',
			   url: $('body').data('abs_client_path')+'BurtlanCtr/viewCart',
			   success: function(data){
				...
				$('.myaccount-page form').ajaxForm({
				  ...
				  beforeSerialize:function(){
				   if($('.myaccount-page form:visible').validate({ignore:'.ignored,:hidden'}).form())
				    $.loader({imgPath:'../assets/Burtlanassets/themes/default/img/loader.gif',appendTo:'body'});
				  },
				  complete:function(){
...
//				  },
//				  dataType:'html',
//				  url:$('body').data('abs_client_path')+'BurtlanCtr/Register',
//				  success:function(data){
			   ...
			   $('#retrieved-password-form').validate();
				...
				$('#retrieve-data-form').ajaxForm({
				  ...
				  beforeSerialize:function(){
						  ...
						  <div style="position:absolute;z-index:9999;visibility:hidden" class="loader-retrieve"><img src="../assets/Burtlanassets/themes/default/img/loader.gif" alt="" /></div>');
						  setTimeout(function(){
...
//				  },
//				  dataType:'html',
//				  url:$('body').data('abs_client_path')+'BurtlanCtr/SaveOrder',
//				  success:function(data){
//				   $.loader.hide();
...
//				});
			...
			$('.add-to-cart-small, .without-options,.add-to-cart').live('click',function(e){
				e.preventDefault();
				$row=$(this).parents('.cartDiv');
				$newValue=$row.find("[name='POL_VALUE']").val();
				if($(this).hasClass('disabled')) return false;
					$type = $(this).hasClass('add-to-cart-small') ? 'small' : 'normal';
					$img = $(this).attr('data-rel-img');
					$price = $(this).attr('data-rel-price');
					...
					$serialize += 'id_product='+$id_product+'&action=add&qta='+$qta+'&imgpath='+$img+'&desc='+$name+'&price='+$price;
					if($qta == 0){
					  $.bootalert({
						  ID        : 'not-available-modal',
						  LabelText : __('general_warning_title'),
					...
					$.ajax({
						type:'POST',
						url: $('body').data('abs_client_path_addCart'),
						beforeSend:function(){
						$.loader({imgPath:'../../assets/Burtlanassets/themes/default/img/loader.gif',appendTo:'body'});
						},
						complete:function(){
						   setTimeout(function(){
								 ...
								 $.loader({
									  //* language-part *//
									  contentToLoad:'<div class="add-to-cart-loader text-center"><button type="button" class="close" onClick=" $.loader.hide();" data-dismiss="modal">&times;</button>\
									  '+__('alert_product_add_to_cart') 
									  .replace(/{name}/g,$name)
									  .replace('{url_img}',$img)
									  .replace('{link_cart}',$('body').data('abs_client_path')+'BurtlanCtr/viewCart')+'\
			...
			$('.product-tbody-container').on('click','.remove-from-cart',function(e){
			    ...
			    $.ajax({
				   type: 'POST',
				   url: $('body').data('abs_client_path_deleteCart'),
				   beforeSend:function(){
				    $.loader({imgPath:'../../assets/Burtlanassets/themes/default/img/loader.gif',appendTo:'body'});
				   },
				   complete: function(){
					$this.closest('tr').fadeOut('fast',function(){
					 $.loader.hide();
					 $(this).remove();
					   if($('.product-tbody-container').find('tr').length <=3){
				   ...
				   success: function(response){
				    ...
				    var itemcode=localStorage.getItem("ITEM_CODE");
				    var itemArray=itemcode.split(",");
				    //console.log(itemArray);
				    var removeItem = $option_product;
				    itemArray = $.grep(itemArray, function(value) {
					return value != removeItem;
				    
				    });
				    //console.log(itemArray);
				    localStorage.setItem("ITEM_CODE",itemArray);
				   // $('.viewCartPage').html(response);
				      update_top_cart();
				   },
			...
			$('.btn-refresh-cart').on('click',function(){
				var input_zero = $('[id^="qta-product_"]').filter(function(){
				 ...
				 $('[name^="qta-product"]').each(function(){
					var $id_product = $(this).attr('data-rel-id-product'),
						...
						arr_options_code['rowid'] = $id_product;
						arr_options_code['qty'] = $value_qta;
					    arr[$i] = arr_options_code;
					    $i++;
						$last_id = $id_product;
				 })*/;
			    $row=$('.bodyOdd');
			    $.ajax({
				   type: 'POST',
				   url: $('body').data('abs_client_path_updateAllCart'),
				   beforeSend:function(){
					 $.loader({imgPath:'../../assets/Burtlanassets/themes/default/img/loader.gif',appendTo:'body'});
				   },
				   complete: function(){
			...
			//alert('aaaaaaaa');
				...
				$option_product = $(this).attr('data-options-item');
				$this = $(this);
				$qta = $('input[data-rel="'+$option_product+'"]').length > 0 ? $('input[data-rel="'+$option_product+'"]').val() : 1;
				if($qta == 0){
				  $.bootalert({
					  ID        : 'not-available-modal',
					  LabelText : __('general_warning_title'),
			    ...
			    $.ajax({
				   type: 'POST',
				   url: $('body').data('abs_client_path_updateCart'),
				   beforeSend:function(){
					 $.loader({imgPath:'../../assets/Burtlanassets/themes/default/img/loader.gif',appendTo:'body'});
				   },
				   complete: function(){
			...
			$('#products-list-table').on('click','table *.increase-qta',function(){
				var $input_qta = $('input#'+$(this).attr('data-products-to-increase'));
			...
			$('#products-list-table').on('click','table *.decrease-qta',function(){
				var $input_qta = $('input#'+$(this).attr('data-products-to-decrease'));
				if($input_qta.val() > 1) $input_qta.val(parseFloat($input_qta.val())-1);
				$this=$(this);
				getPriceUpdate($(this));
				
				$row=$this.parents('.odd');
				  textVal=$row.find('[name="qta-product[]"]').val();
				  flexVal=$row.find('[name="POL_FLEX_01[]"]').val();
				if (textVal>=flexVal){
				    $('input#'+$(this).attr('disabled',false));
				}
				else{
				    var $input_qta = $('input#'+$(this).attr('data-products-to-decrease'));
				    $row.find('input[name="qta-product[]"]').val(flexVal);
			...
			function get($this){
			    ...
			    textVal=$row.find('[name="qta-product[]"]').val();
			    flexVal=$row.find('[name="POL_FLEX_01[]"]').val();
			    if (parseFloat(textVal)<parseFloat(flexVal)){
				$value=$row.find('[name="qta-product[]"]').val(flexVal);
				getPriceUpdate($value);
			    }
			    else{
				getPriceUpdate($this);
			    }
			...
			//names=itemArray;
			function change($this,ITEM_CODE){
			    $row=$this.parents('.cartDiv');
			    buttonOld=$row.find('[name="addCart"]');
			    buttonOld.hide();
			    buttonNew=$row.find('[name="newButton"]');
			    buttonNew.removeClass('hidden');
			    //buttonNew.show();
			    //buttonNew.removeAttr('style');
			    names.push(ITEM_CODE);
			    localStorage.setItem("ITEM_CODE",names);
			}
			
	    		function getPriceUpdate($this){
			   $row=$this.parents('.odd');
			   price=parseFloat($row.find('input[name="POL_PRICE_U[]"]').val().replace(",",""));
			   qty=$row.find('input[name="qta-product[]"]').val();
			   POL_VALUE=qty*price;
			   //alert(POL_VALUE);
			  $row.find('input[name="POL_VALUE_U[]"]').val(POL_VALUE);
			  if (POL_VALUE>0) {
			  
			   POL_DISC_PCT=parseFloat($row.find('input[name="POL_DISC_PCT_U[]"]').val().replace(",",""));
			   POL_DISC_VALUE=0;
			  if (POL_DISC_PCT!=0) {
			  POL_DISC_VALUE=POL_VALUE * POL_DISC_PCT/100;
			  $row.find('[name="POL_DISC_VALUE_U[]"]').val(POL_DISC_VALUE);
			  }
			   POL_TAX_PCT=parseFloat($row.find('input[name="POL_TAX_PCT_U[]"]').val().replace(",",""));
			   POL_TAX_VALUE=0;
			  if (POL_TAX_PCT!=0){
			  POL_TAX_VALUE=(POL_VALUE - POL_DISC_VALUE)*POL_TAX_PCT/100;
			  $row.find('[name="POL_TAX_VALUE_U[]"]').val(POL_TAX_VALUE);
			  }
			   POL_GROSS_VALUE =(POL_VALUE-POL_DISC_VALUE)+POL_TAX_VALUE;
			  $row.find('[name="POL_GROSS_VALUE_U[]"]').val( POL_GROSS_VALUE);
			  // POH_EXC_RATE=$('#POH_EXC_RATE').val();
			   POH_EXC_RATE=1;
			   POL_GROSS_VALUE_LC =(POL_GROSS_VALUE*POH_EXC_RATE);
			  $row.find('[name="POL_GROSS_VALUE_LC_U[]"]').val(POL_GROSS_VALUE_LC);
			  
			  // tot_disc_value=0;
			  //$("[name='POL_DISC_VALUE_U[]']").each(function(){
			  //if ($(this).val()!="") {
			  //tot_disc_value=parseFloat(tot_disc_value)+parseFloat($(this).val().replace(",",""));
			  //}
			  //});
			  //$('#POH_DISCOUNT_VALUE').val(tot_disc_value);
			  //$('#tot_disc_value').val(tot_disc_value);
			  // tot_tax_value=0;
			  //$("[name='POL_TAX_VALUE_U[]']").each(function(){
			  //if ($(this).val()!="") {
			  //tot_tax_value=parseFloat(tot_tax_value)+parseFloat($(this).val().replace(",",""));
			  //}
			  //});
			  //$('#POH_TAX_VALUE').val(tot_tax_value);
			  //$('#tot_tax_value').val(tot_tax_value);
			  // tot_gross_value=0;
			  //$("[name='POL_GROSS_VALUE_U[]']").each(function(){
			  //if ($(this).val()!="") {
			  //tot_gross_value=parseFloat(tot_gross_value)+parseFloat($(this).val().replace(",",""));
			  //}
			  //});
			  //$('#POH_GROSS_VALUE').val(tot_gross_value);
			  //$('#tot_gross_value').val(tot_gross_value);
			  // POH_gross_value_lc=(tot_gross_value*POH_EXC_RATE);
			  //$('#POH_GROSS_VALUE_LC').val(POH_gross_value_lc);
			...
			$('[data-steps-rel^="fstep_"]:last').on('classadded', function(ev, newClasses) {
			  if(newClasses == 'current-step'){
				  var $billing_address = get_address($('#POH_DLV_LOCN_CODE').val(),$('#POH_PT_CODE').val(),$('#POH_CARRIER_CODE').val(),$('#POH_FREIGHT_CODE').val()),
